...
#' 
#' Compares TWFE, Matrix Completion, Synthetic DiD, and TROP estimators
#' under various DGP configurations with latent factor structure.
#' ============================================================================
...
run_one_sim <- function(
  ...
  if (is.null(data)) {
    return(tibble(
      sim_id = sim_id,
      method = c("TWFE", "Matrix Completion", "Synthetic DiD", "TROP")[
        c("twfe", "mc", "sdid", "trop") %in% estimators
      ],
      estimate = NA_real_,
      se = NA_real_,
      p_value = NA_real_,
  ...
  if ("sdid" %in% estimators) {
    ...
    results <- bind_rows(results, tibble(
      sim_id = sim_id, method = "Synthetic DiD", estimate = res$estimate, se = res$se,
      p_value = res$p_value, ci_lower = res$ci_lower, ci_upper = res$ci_upper,
      true_att = true_att
    ))
  }
